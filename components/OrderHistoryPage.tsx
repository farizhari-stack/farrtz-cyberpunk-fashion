...
const OrderHistoryPage: React.FC<OrderHistoryPageProps> = ({ onNavigate, user, onAccountClick, cartItemCount, onLogout, onSearch }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            if (user) {
                const userOrders = await authService.getOrdersByUser(user.id);
                ...
                {steps.map((step, idx) => {
                    ...
                    return (
                        <div key={idx} className="flex flex-col items-center gap-2">
                            <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full border-2 transition-all duration-500 ${isActive ? 'bg-white border-cyan-400 shadow-[0_0_10px_#22d3ee]' : 'bg-gray-900 border-gray-600'
                                }`}>
                                {isCurrent && (
                                    <div className="absolute top-0 left-0 w-full h-full bg-cyan-400 rounded-full animate-ping opacity-75"></div>
                                )}
                            </div>
                            <span className={`text-[10px] md:text-xs font-bold uppercase tracking-wider transition-colors ${isActive ? 'text-white' : 'text-gray-500'
                                }`}>
                                {step.label}
                            </span>
                        </div>
